...
export class AssetService {
    ...
    getAsset(qaTypeCode: string): Observable<AssetEntity> {
        return this.httpClient.get<AssetEntity>(this.str + '/Asset/' + qaTypeCode).pipe(catchError(this.handleError));
    }
    Save(saveEntityObj: AssetEntity): Observable<any> {
        console.log('service');
        console.log(AssetEntity);
        const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) };
        return this.httpClient.post<any>(this.str + '/Asset', saveEntityObj, httpOptions).pipe(catchError(this.handleError));
    }

    Update(updateEntityObj: AssetEntity): Observable<Insertstatus> {
        const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) };
        // tslint:disable-next-line:max-line-length
        return this.httpClient.post<Insertstatus>(this.str + '/Asset', updateEntityObj, httpOptions).pipe(catchError(this.handleError));
    }
